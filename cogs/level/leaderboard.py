...
class leaderboard(commands.Cog):
    ...
    async def leaderboard(self, ctx, leader_type=None):
        if leader_type is None:
            ...
            embed = nextcord.Embed(title=f":trophy: {ctx.guild.name}'s Leaderboard | Top {con}", color = config['leaderboard_embed_color'])
            i = 1
            for x in rankings:
                try:
                    ...
                    embed.add_field(name=f"#{i}: {temp.name}", value=f"Level: `{level}`\nTotal XP: `{xp}`\n", inline=True)
                    i += 1
        ...
        if leader_type.lower() == 'global':
            ...
            con = config['leaderboard_amount']
            embed = nextcord.Embed(title=f"🌎 Global Leaderboard | Top {con}", color=config['leaderboard_embed_color'])
            i = 1
            for x in rankings:
                try:
                    tempxp  =x["xp"]
