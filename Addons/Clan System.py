...
class Clan(commands.Cog):
    ...
    async def clan(self, ctx, state=None):
        ...
        if state is None:
            embed = nextcord.Embed(title="👑 | CLAN SYSTEM", description="Welcome to the Clan System. Invite friends to earn xp simultaneously when someone from the same clan sends a message.")
        ...
        if state:
            ...
            if clan_search:
                ...
                embed = nextcord.Embed(title=f"{state.upper()} CLAN | {str(clan_search['status']).upper()}")
                ...
                embed = nextcord.Embed(title=":x: There was an Error!",
    ...
    async def create(self, ctx, name=None, publicity=None):
        owner = ctx.author
        if name is None:
            embed = nextcord.Embed(title=":x: There was an Error!", description="*You need to define a name for the clan!*")
            await ctx.send(embed=embed)
            return
        ...
        if publicity.lower() != 'public' and publicity.lower() != 'private':
            ...
            level = stats['rank']
            if user_search:
                embed = nextcord.Embed(title=":x: There was an Error!", description="`You already own a clan!`")
                await ctx.send(embed=embed)
                return
            if user_in_clan_search:
                embed = nextcord.Embed(title=":x: There was an Error!", description="`You are already in a clan!`")
                await ctx.send(embed=embed)
                return
            if int(level) < int(config['level_for_clan']):
                embed = nextcord.Embed(title=":x: There was an error!", description=f"*You aren't the right level to create a clan! Level up `{config['level_for_clan']-level}` times!*")
                await ctx.send(embed=embed)
                return
            if clan_search:
                embed = nextcord.Embed(title=":x: There was an error!", description=f"The clan `{name}` already exists!")
                await ctx.send(embed=embed)
                return
            else:
                ...
                embed = nextcord.Embed(title=f"{name.upper()} CLAN", description=f"You have successfully created a clan with the name `{name.lower()}`!")
    ...
    async def invite(self, ctx, member: nextcord.Member = None):
        ...
        if member is None:
            embed = nextcord.Embed(title=":x: There was an Error!", description="`You need to define a user to invite!`")
            await ctx.send(embed=embed)
            return
        ...
        if status == 'public':
            ...
            return
        if member:
            embed = nextcord.Embed(title=f"CLAN INVITE | {clan_search['clan_name']}", description=f"You have received an invitation from {ctx.author.mention} to join `{clan_search['clan_name']}`")
            ...
            embed = nextcord.Embed(description=f"Invite sent to `{member.name}`! They have `5` Minutes to Accept.")
            await ctx.send(embed=embed)
            ...
            while True:
                try:
                    ...
                    if str(reaction.emoji) == f"❌":
                        embed = nextcord.Embed(description=f"{ctx.author.mention}, {member.mention} declined your clan invite to `{clan_search['clan_name']}`")
    ...
    async def join(self, ctx, clan=None):
        if clan is None:
            embed = nextcord.Embed(title=":x: There was an Error!", description="`You need to specify which clan to join!`")
            await ctx.send(embed=embed)
        elif clan:
            ...
            if user_search:
                ...
                if clan_search:
                    if len(clan_search['users']) >= 10:
                        embed = nextcord.Embed(title=":x: There was an Error!", description=f"The clan `{clan}` is full!")
                        await ctx.send(embed=embed)
                        return
                    if clan_search['status'] == "private":
                        embed = nextcord.Embed(title=":x: There was an Error!", description=f"The clan `{clan}` is `private`!")
                        await ctx.send(embed=embed)
                        return
                    else:
                        levelling.update({"guildid": ctx.guild.id, "clan_name": clan_search['clan_name']},
                                         {"$push": {"users": f"{ctx.author}"}})
                        embed = nextcord.Embed(title=f":white_check_mark: You joined the clan `{clan_search['clan_name']}`!")
                        await ctx.send(embed=embed)
                else:
                    embed = nextcord.Embed(title=":x: There was an Error", description=f"The clan `{clan}` does not exist!")
                    await ctx.send(embed=embed)
    ...
    async def status(self, ctx):
        ...
        if clan_search['status'] == "private":
            ...
            embed = nextcord.Embed(title=f":white_check_mark: Status for `{clan_search['clan_name']}` set to `Public`")
            await ctx.send(embed=embed)
        else:
            levelling.update_one({"guildid": ctx.guild.id, "owner": ctx.author.id}, {"$set": {"status": "private"}})
            embed = nextcord.Embed(title=f":white_check_mark: Status for `{clan_search['clan_name']}` set to `Private`")
            await ctx.send(embed=embed)
    ...
    async def delete(self, ctx):
        ...
        if clan_search:
            ...
            embed = nextcord.Embed(title=f":white_check_mark: The clan `{clan_search['clan_name']}` was successfully deleted!")
            await ctx.send(embed=embed)
        else:
            embed = nextcord.Embed(title=":x: There was an Error", description="You do not own a clan!")
            await ctx.send(embed=embed)
    ...
    async def leave(self, ctx):
        ...
        if clan_search:
            if ctx.author.id == clan_search['owner']:
                embed = nextcord.Embed(title=":x: There was an Error!", description="`You are the owner of the clan, you must delete it to leave!`")
                await ctx.send(embed=embed)
                return
            levelling.update({"guildid": ctx.guild.id, "clan_name": clan_search['clan_name']},
                             {"$pull": {"users": f"{ctx.author}"}})
            embed = nextcord.Embed(title=f":white_check_mark: You have left the clan `{clan_search['clan_name']}`")
            await ctx.send(embed=embed)
    ...
    async def clans(self, ctx):
        ...
        if clan_search:
            ...
            embed = nextcord.Embed(title=f":video_game: PUBLIC CLANS | `10 RESULTS`",
            ...
            embed = nextcord.Embed(title=":video_game: PUBLIC CLANS | `NO RESULTS`", description="🔴 There are currently no `public` clans! Try again later, or create one.")
            await ctx.send(embed=embed)
