...
class seasonsys(commands.Cog):
                ...
                if datetime.now().day == config['halloween_start_date']:
                    if bot_stats['event_state'] is True:
                        ...
                        for guild in active_servers:
                            ...
                            embed = nextcord.Embed(title=":jack_o_lantern: // **Spook Season**", description=f"Trick or Treat? Word has it, <@{self.client.user.id}> has the best treats at `🏚️ House #{house_number}`", colour=0xF75F1C)
            ...
            if datetime.now().month == 12:
                if datetime.now().day == config['christmas_start_date']:
                    if bot_stats['event_state'] is True:
                        ...
                        for guild in active_servers:
                            serverstats = levelling.find_one({"server": guild.id})
                            embed = nextcord.Embed(title=":christmas_tree: // **Christmas Holiday**", description=f"Tis the season to be jolly! <@{self.client.user.id}> spreads Christmas cheer to all!", colour=0x0F7833)
            ...
            if datetime.now().month == 7:
                if datetime.now().day == config['summer_start_date']:
                    if bot_stats['event_state'] is True:
                        ...
                        for guild in active_servers:
                            serverstats = levelling.find_one({"server": guild.id})
                            embed = nextcord.Embed(title=":beach: // **Summer Holiday**", description=f"High tide or low tide, <@{self.client.user.id}> will stay by your side", colour=0xFFFE6F)
    ...
    async def countdown(self, ctx):
        ...
        if month == "November" or month == "December":
            ...
            strSeconds = str(secs)
            christmas_embed = nextcord.Embed(title="🎅 CHRISTMAS COUNTDOWN!", description=f"The Christmas season is coming, so to get into the spirit, here's a Christmas Countdown.", colour=0xc54245)
            ...
            christmas_embed.set_image(url="https://www.rd.com/wp-content/uploads/2018/10/this-is-why-christmas-is-on-deember-25.jpg?resize=768,512")
            christmas_embed.set_thumbnail(url="https://cdn.nextcordapp.com/attachments/831180817064656907/890737461033054268/Santa_PNG_Clipart-54.png")
            ...
            strMinutes = str(minutes)
            strSeconds = str(secs)
            halloween_embed = nextcord.Embed(title="🎃 HALLOWEEN COUNTDOWN!", description=f"The Spooky season season is coming, so to begin the scares, here's a Halloween Countdown.", colour=0xeb6123)
            ...
            halloween_embed.set_image(url="https://cdn.pixabay.com/photo/2017/10/10/16/55/halloween-2837936_960_720.png")
            ...
            strMinutes = str(minutes)
            strSeconds = str(secs)
            easter_embed = nextcord.Embed(title="🐰 EASTER COUNTDOWN!", description=f"The Egg Hunts begin soon! For the mean time, here's an Easter Countdown.", colour=0xe9aad1)
