...
class Extras(commands.Cog):
    ...
    async def addxp(self, ctx, xpamount=None, member=None):
        ...
        if xpamount:
            ...
            embed = nextcord.Embed(title=":white_check_mark: **ADDED XP!**",
    ...
    async def removexp(self, ctx, xpamount=None, member=None):
        ...
        if xpamount:
            xp = stats["xp"]
            levelling.update_one({"guildid": ctx.guild.id, "tag": userget}, {"$set": {"xp": xp - int(xpamount)}})
            embed = nextcord.Embed(title=":white_check_mark: **REMOVED XP!**",
    ...
    async def reset(self, ctx, user=None):
        if user:
            ...
            embed = nextcord.Embed(title=f":white_check_mark: RESET USER", description=f"Reset User: {user}",
                                  color=config['success_embed_color'])
            print(f"{userget} was reset!")
            await ctx.send(embed=embed)
        else:
            prefix = config['Prefix']
            embed2 = nextcord.Embed(title=f":x: RESET USER FAILED",
    ...
    #         embed = nextcord.Embed(title=f"{self.client.user.name} Command List")
    #         if helptype is None:
    #             embed.add_field(name=f":camera: Profile",
    #                             value=f'`{prefix}help profile`\n[Hover for info](https://www. "Customise your rank cards background, xp color and your profile pictures shape!")')
    ...
    #             embed = nextcord.Embed(title=f":camera: Profile Commands",
    #                                   description="```background, circlepic, xpcolor```")
    ...
    #                                   f"Profile Picture\n🎨 {prefix}xpcolor <hex> - Changes your XP progress bar's "
    #                                   f"color```")
    #             await ctx.send(embed=embed)
    #         elif helptype.lower() == "fun":
    #             embed = nextcord.Embed(title=f":smile: Fun Commands",
    ...
    #                 embed = nextcord.Embed(title=f":octagonal_sign: Admin Commands",
    ...
    #             if ctx.message.author.guild_permissions.administrator:
    #                 embed = nextcord.Embed(title=f":gear: Config Commands",
    ...
    #             embed = nextcord.Embed(title=f"👥 Clan Commands",
