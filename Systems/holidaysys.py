        ...
        if ctx.message.author.id != config2['bot_owner_id']:
            ...
            if holiday == "Christmas":
                if state == "start":
                    for guild in self.client.guilds:
                        ...
                        embed = nextcord.Embed(title=":christmas_tree: **Christmas Holiday**",
                                              description=f"'Tis the season to be jolly! <@{self.client.user.id}> spreads Christmas cheer to all!", color=0x0F7833)
                    ...
                    for guild in self.client.guilds:
                        ...
                        embed = nextcord.Embed(title=":christmas_tree: **Christmas Holiday Over!**",
                                              description=f"The Christmas season is over.. We hope you had a great holiday full of joy!\n\n:gift: The **Present** `x{config['bonus_xp']}xp` has been reverted.", color=0x0F7833)
                        await guild.text_channels[0].send(embed=embed)
                else:
                    embed = nextcord.Embed(title=":x: ***SOMETHING WENT WRONG!***", description="State must be either `start` or `stop`!")
                ...
                if state == "start":
                    for guild in self.client.guilds:
                        ...
                        embed = nextcord.Embed(title=":jack_o_lantern: ***Spook Season***",
                                              description=f"Trick or Treat? Word has it, <@{self.client.user.id}> has the best treats at :house_abandoned: #{house_number}", color=0xF75F1C)
                ...
                if state == "start":
                    for guild in self.client.guilds:
                        levelling.update_one({"server": guild.id}, {"$set": {"event": "Started"}})
                        summer_embed = nextcord.Embed(title=":beach: **Summer Holiday**",
                                                     description=f"High tide or low tide, <@{self.client.user.id}> will stay by your side", color=0xFFFE6F)
                    ...
                    for guild in self.client.guilds:
                        levelling.update_one({"server": guild.id}, {"$set": {"event": "Ended"}})
                        embed = nextcord.Embed(title=":beach: **Summer Holiday Over!**",
                                              description=f"The tides are down and the heat is going.. We hope you had a great summer!\n\n:icecream: The **Gift** `x{config['bonus_xp']}xp` has been reverted.", color=0xFFFE6F)
                        await guild.text_channels[0].send(embed=embed)
                    return
                else:
                    embed = nextcord.Embed(title=":x: ***SOMETHING WENT WRONG!***", description="State must be either `start` or `stop`!")
                ...
                if state == "start":
                    for guild in self.client.guilds:
                        levelling.update_one({"server": guild.id}, {"$set": {"event": "Started"}})
                        summer_embed = nextcord.Embed(title=":rabbit: **Easter Holiday**",
                                                     description=f"The Egg Hunt begins, <@{self.client.user.id}> has hidden Eggs all around your garden!", color=0xee82ee)
                ...
                embed = nextcord.Embed(title=":x: ***SOMETHING WENT WRONG!***", description=f"`{holiday}` is not a valid holiday! Please chose from the list below:")
