...
from ruamel.yaml.compat import DBG_EVENT
import vacefron
import os
import re
...
if os.path.exists("Configs/holidayconfig.yaml") is True:
    ...
    async def on_message(self, ctx):
        ...
        if not ctx.author.bot:
            if stats is None:
                ...
                # Used as a backup in cae on_guild_join fails
                member = ctx.author
                user = f"<@{member.id}>"
                newuser = {
                            "guildid" : ctx.guild.id,
                ...
                if len(talk_channels) > 1 and ctx.channel.id not in talk_channels or config['Prefix'] in ctx.content:
                    ...
                    if role in user.roles:
                        ...
                        if bot_stats["event_state"] is True:
                            ...
                            xp = stats['xp'] + serverstats['xp_per_message'] * holidayconfig['bonux_xp']
                            levelling.update_one(
                                {
                                    "guildid" : ctx.guild.id,
                                    "id" : ctx.author.id
                        ...
                        user = ctx.author
                        role = nextcord.utils.get(ctx.guild.roles, name=serverstats["duble_xp_roles"])
                        if role in user.roles:
                            stats = levelling.find_one(
                                {
                                    "guildid" : ctx.guild.id,
                    ...
                    levelling.update_one(
                        ...
                        },
                        {
                            ...
                            {
                                "rank" : lvl,
                                "xp" : xp + serverstats['xp_per_message'] * 2
                            }
                        }
                    )
                    ...
                    for i in range(len(level_roles)):
                        ...
                        embed = nextcord.Embed(title=":tada: **LEVEL UP!**",
                                            description=f"{ctx.author.mention} just reached Level: **{lvl}**",
                                            color=config['embed_color'])
                        embed.add_field(name="Next Level:", value=f"{int(config['xp_per_level'] * 2 * ((1 / 2) * lvl))}xp")
    ...
    async def on_guild_join(self, guild):
        ...
        if serverstats is None:
            ...
            if config['private_message'] is True:
                ...
                embed = nextcord.Embed(title=f":wave: // Greetings, {guild.name}", description=f"Thanks for inviting me, my prefix is: `{prefix}`")
                if os.path.exists("Addons/Extras+.py") is True:
                    embed.add_field(name=":rocket: | What's Next?", value=f"`{prefix}help` displays every command you need to know for {self.client.user.mention}", inline=False)
