...
class spamsys(commands.Cog):
    ...
    async def on_message(self, ctx):
        ...
        if serverstats['Antispam'] is True:
            member = ctx.author
            role = nextcord.utils.get(member.guild.roles, name=serverstats['ignoredRole'])
            ...
            # If a user ignored the warning, they'll receive a mute
            if warnings == mutemessage - 1:
                takeaway = serverstats['xp_per_message'] * serverstats['muteMessages']
                xp  =stats['xp'] - takeaway
                membed = ctx.author
                rank = nextcord.utils.get(member.guild.roles, name=serverstats['muteRole'])
                
                await member.add_roles(rank) # Added the muted role
    ...
    async def antispamstats(self, ctx):
        serverstats = levelling.find_one({"server": ctx.guild.id})
        embed2 = nextcord.Embed(title=":warning: Anti-Spam", description="Anti-Spam System `v1.5`", color=0xFFCC00)
