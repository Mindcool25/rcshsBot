...
class spamsys(commands.Cog):
    ...
    # Everytime a user sends a message, it'll add it to the spam file and after every x seconds it will clear
    @commands.Cog.listener()
    async def on_message(self, ctx):
        counter = 0
        serverstats = levelling.find_one(
            {
                "server" : ctx.guild.id
            }
        )
        ...
        if serverstats['Antispam'] is True:
            member = ctx.author
            role = nextcord.utils.get(member.guild.roles, name=serverstats['ignoredRoles'])
            ...
            if warnings == warningmessage - 1:
                embed = nextcord.Embed(title=f":warning: WARNING",
            ...
            if warnings == mutemessage - 1:
                ...
                member = ctx.author
                rank = nextcord.utils.get(member.guild.roles, name=serverstats['mutedRole'])
                ...
                print(f"User: {ctx.author} has now been unmuted.")
                levelling.update_one(
                    {
                        "guildid" : ctx.guild.id,
                        "id" : ctx.author.id
    ...
    async def antispamstats(self, ctx):
        ...
        embed2 = nextcord.Embed(title=":warning: Anti-Spam", description="Anti-Spam System v0.1", color=0xFFCC00)
    ...
    async def exit_handler(self, ctx):
        ...
        for member in ctx.guild.members:
            user = ctx.author
            role = nextcord.utils.get(ctx.guild.roles, name=serverstats["mutedRole"])
            if role in user.roles:
                await member.remove_roles(role)

# Sets-up the cog for antispam
def setup(bot):
    bot.add_cog(spamsys(bot))